...
public class EncryptionUtil {
    ...
    public static byte[] decryptBytes(byte[] encryptedData) {
        try {
            ...
            // This happens if the key/IV is wrong or the file is not encrypted with this
            // key
            System.err.println(
                    "⚠️ Decryption failed: Invalid key or corrupted data (BadPaddingException). Falling back to raw bytes.");
            return null;
        } catch (Exception ex) {
            ex.printStackTrace();
            return null;
