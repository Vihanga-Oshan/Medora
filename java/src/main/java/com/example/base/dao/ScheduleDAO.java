...
public class ScheduleDAO {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    for (String time : timesOfDay) {
                        ...
                        m.setInstructions(EncryptionUtil.decrypt(rs.getString("instructions")));
    ...
    public boolean updateMedicationSchedule(int id, String dosage, String frequency,
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            stmt.setString(6, EncryptionUtil.encrypt(instructions));
            stmt.setInt(7, id);
    ...
    public java.util.Map<String, Integer> getAdherenceStats(String patientNic) throws SQLException {
        ...
        String sql = "SELECT status, COUNT(*) as count FROM medication_log WHERE patient_nic = ? GROUP BY status";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, patientNic);
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    if ("TAKEN".equalsIgnoreCase(status)) {
                        stats.put("taken", count);
                    } else if ("MISSED".equalsIgnoreCase(status)) {
                        stats.put("missed", count);
                    } else {
        ...
        stats.put("total", stats.get("taken") + stats.get("missed") + stats.get("pending"));
        return stats;
    }
}
