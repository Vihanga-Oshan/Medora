...
public class AdminDAO {
    ...
    public boolean registerAdmin(Admin admin) {
        try {
            ...
            stmt.setString(1, EncryptionUtil.encrypt(admin.getEmail()));
            ...
            stmt.setString(5, EncryptionUtil.encrypt(admin.getContactNumber()));
    ...
    public Admin validate(String email, String password) {
        try {
            ...
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setString(1, EncryptionUtil.encrypt(email));
            stmt.setString(2, hashedPassword);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                Admin admin = new Admin();
                admin.setId(rs.getInt("id"));
                admin.setEmail(EncryptionUtil.decrypt(rs.getString("email")));
                admin.setFullName(rs.getString("name"));
                admin.setNic(rs.getString("nic"));
                admin.setContactNumber(EncryptionUtil.decrypt(rs.getString("contact")));
                return admin;
            }
        } catch (SQLException e) {
            e.printStackTrace();
