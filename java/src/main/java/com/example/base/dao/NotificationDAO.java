...
public class NotificationDAO {
    ...
    public List<Notification> getNotificationsByGuardianNic(String guardianNic) throws SQLException {
        List<Notification> list = new ArrayList<>();
        // Fetch notifications for all patients linked to this guardian
        String sql = "SELECT * FROM notifications WHERE patient_nic IN (SELECT nic FROM patient WHERE guardian_nic = ?) ORDER BY created_at DESC";
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
