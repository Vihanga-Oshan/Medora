...
public class PrescriptionFileServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        // ✅ Stream file (Decrypted)
        try {
            byte[] encryptedData = java.nio.file.Files.readAllBytes(file.toPath());
            byte[] decryptedData = com.example.base.util.EncryptionUtil.decryptBytes(encryptedData);

            if (decryptedData == null) {
                // If decryption fails, fallback to original (might be legacy unencrypted file)
            ...
            LOGGER.log(Level.SEVERE, "Error streaming decrypted file: " + file.getAbsolutePath(), e);
            if (!resp.isCommitted())
