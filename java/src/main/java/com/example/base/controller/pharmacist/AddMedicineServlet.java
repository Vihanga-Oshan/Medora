...
public class AddMedicineServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        if (filePart != null && filePart.getSize() > 0) {
            ...
            // Encrypt and save to webapp deployment directory
            try (java.io.InputStream input = filePart.getInputStream()) {
                byte[] fileData = input.readAllBytes();
                byte[] encryptedData = com.example.base.util.EncryptionUtil.encryptBytes(fileData);
                if (encryptedData != null) {
                    try (java.io.FileOutputStream output = new java.io.FileOutputStream(
                            savePath + java.io.File.separator + fileName)) {
                        output.write(encryptedData);
                    }
                } else {
                    filePart.write(savePath + java.io.File.separator + fileName); // Fallback to plain if encrypt fails
                }
            }
            ...
            try {
                ...
                if (sourceDir.exists()) {
                    try (java.io.InputStream input = filePart.getInputStream()) {
                        ...
                        if (encryptedData != null) {
                            try (java.io.FileOutputStream output = new java.io.FileOutputStream(
                                    sourcePath + java.io.File.separator + fileName)) {
                                output.write(encryptedData);
                            }
                        }
                    }
            ...
            // Set the path to point to our decryption servlet
            imagePath = req.getContextPath() + "/medicineImage/" + fileName;
        } else {
        ...
        if (error != null) {
            ...
            reloadFormAfterError(req, resp);
            return;
        }
        // --- End Validation ---

        Object jwtSub = req.getAttribute("jwtSub");
        if (jwtSub == null) {
            resp.sendRedirect(req.getContextPath() + "/pharmacist/login");
            return;
        }

        int pharmacistId = Integer.parseInt(jwtSub.toString());
    ...
    private void loadDropdownData(HttpServletRequest req, Connection conn) throws SQLException {
        req.setAttribute("categories", CategoryDAO.getAll(conn));
        req.setAttribute("dosageForms", com.example.base.dao.DosageFormDAO.getAll(conn));
        req.setAttribute("sellingUnits", com.example.base.dao.SellingUnitDAO.getAll(conn));
    }

    private void reloadFormAfterError(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        try (Connection conn = dbconnection.getConnection()) {
            loadDropdownData(req, conn);
            req.getRequestDispatcher("/WEB-INF/views/pharmacist/add-medicine.jsp").forward(req, resp);
        } catch (Exception e) {
            LOGGER.log(Level.SEVERE, "Error reloading form", e);
            resp.sendError(500);
        }
    }
