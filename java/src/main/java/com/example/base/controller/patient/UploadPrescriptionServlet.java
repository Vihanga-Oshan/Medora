...
public class UploadPrescriptionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        // ✅ Save the uploaded file (Encrypted)
        try (java.io.InputStream input = filePart.getInputStream()) {
            ...
            if (encryptedData != null) {
                try (java.io.FileOutputStream output = new java.io.FileOutputStream(filePath)) {
                    output.write(encryptedData);
                }
                LOGGER.info("📁 Saved encrypted prescription file: " + filePath);
            } else {
                filePart.write(filePath);
                LOGGER.warning("⚠️ Encryption failed, saved prescription file as plain text: " + filePath);
            }
        }
