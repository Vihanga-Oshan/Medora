...
public class PatientDashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        try (Connection conn = dbconnection.getConnection()) {
            ...
            // NEW: Check for Guardian Link Request
            patientDAO dao = new patientDAO(conn);
            String linkStatus = dao.getLinkStatus(patientNic);

            if ("REQUEST_SENT".equals(linkStatus)) {
                // Determine who is requesting.
                // Since there's only one 'guardian_nic' in the patient table,
                // and 'REQUEST_SENT' implies THAT guardian requested it.
                // We need to fetch that guardian's details.
                // For now, let's just get the NIC from the patient object.
                patient p = dao.getPatientByNIC(patientNic);
                if (p != null && p.getGuardianNic() != null) {
                    // Ideally we fetch Guardian Name using GuardianDAO.
                    // Since GuardianDAO is available, let's use it if possible.
                    Guardian g = GuardianDAO.getGuardianByNic(p.getGuardianNic());
                    if (g != null) {
                        req.setAttribute("requestingGuardian", g);
                    }
                }
            }
