...
<head>
    ...
    <link rel="stylesheet"
        href="<?= htmlspecialchars($base) ?>/assets/css/guardian/patients-modern.css?v=<?= $cssVer ?>">
...
<body class="guardian-body">
    ...
    <div class="dashboard-wrapper">
        <header class="dashboard-hero guardian-patients-hero">
            <div class="hero-content page-hero-content">
                ...
                <p class="page-hero-subtitle">Check patient updates and today&apos;s medication progress in one place.
                </p>
            </div>
                ...
                <aside class="card-panel linked-patients-panel">
                    <div class="section-header">
                        <div>
                            ...
                            <p class="section-copy">Quickly switch between profiles and monitor current medication
                                status.</p>
                        </div>
                    ...
                    <?php if (empty($patients)): ?>
                        <div class="empty-state-modern linked-empty">
                            ...
                            <p>Use a patient NIC to send a link request and start monitoring their schedule once they
                                approve.</p>
                            <button type="button" class="btn btn-primary" onclick="openAddModal()">Link First
                                Patient</button>
                        </div>
                    <?php else: ?>
                        <div class="linked-patient-list">
                            <?php foreach ($patients as $p): ?>
                                ...
                                ?>
                                <article class="linked-patient-card <?= $isActive ? 'active' : '' ?>"
                                    onclick="window.location.href='<?= htmlspecialchars($base) ?>/guardian/patients?nic=<?= urlencode($p['nic']) ?>'">
                                    <div class="linked-patient-main">
                                        <div class="item-avatar"><?= htmlspecialchars(strtoupper(substr($displayName, 0, 1))) ?>
                                        </div>
                                        <div class="linked-patient-copy">
                                            ...
                                            <p class="patient-card-meta">
                                                <?= htmlspecialchars($gender) ?>         <?php if ($chronicIssues !== ''): ?> |
                                                    <?= htmlspecialchars($chronicIssues) ?>         <?php endif; ?>
                                            </p>
                                        </div>
                                    </div>
                                    <div class="linked-patient-actions">
                                        <a href="<?= htmlspecialchars($base) ?>/guardian/patients?nic=<?= urlencode($p['nic']) ?>"
                                            class="mini-link" onclick="event.stopPropagation()">Open</a>
                                        <form action="<?= htmlspecialchars($base) ?>/guardian/patients/remove" method="post"
                                            onsubmit="event.stopPropagation(); return confirm('Remove patient from guardian list?');">
                            ...
                            <div class="patient-spotlight-header">
                                <div class="patient-identity">
                                    <div class="big-avatar">
                                        <?= htmlspecialchars(strtoupper(substr((string) $selectedPatient['name'], 0, 1))) ?>
                                    </div>
                                    <div>
                                        <h2><?= htmlspecialchars($selectedPatient['name']) ?></h2>
                                        <p class="patient-subtitle">NIC <?= htmlspecialchars($selectedPatient['nic']) ?> |
                                            <?= htmlspecialchars((string) ($selectedPatient['gender'] ?? 'Not specified')) ?>
                                        </p>
                                    </div>
                                </div>
                        ...
                        <div class="card-panel">
                            <div class="section-header">
                                <div>
                                    ...
                                    <p class="section-copy">A guardian-friendly view of the patient's current schedule and
                                        completion status.</p>
                                </div>
                                <span class="date-badge">&#128197;
                                    <span><?= htmlspecialchars($selectedDate) ?></span></span>
                            </div>

                            <div class="schedule-list">
                                <?php if (empty($medications)): ?>
                                    <div class="empty-state-modern">
                                        <div class="state-icon state-icon-blue"><img
                                                src="<?= htmlspecialchars($base) ?>/assets/img/pill.jpg" alt=""
                                                aria-hidden="true"
                                                style="width:24px;height:24px;object-fit:contain;display:block;"></div>
                                ...
                                <?php else: ?>
                                    <?php foreach ($medications as $m): ?>
                                        ...
                                        <article class="guardian-medication-item <?= htmlspecialchars($statusClass) ?>">
                                            <div class="med-time">
                                                <span
                                                    class="time-slot"><?= htmlspecialchars((string) ($m['frequency'] ?? '-')) ?></span>
                                                <span
                                                    class="meal-timing"><?= htmlspecialchars((string) ($m['meal_timing'] ?? 'Anytime')) ?></span>
                                            </div>
                                            <div class="med-info-block">
                                                <h3><?= htmlspecialchars((string) ($m['medicine_name'] ?? 'Medication')) ?></h3>
                                                <p class="dosage-info"><img src="<?= htmlspecialchars($base) ?>/assets/img/pill.jpg"
                                                        alt="" aria-hidden="true"
                                                        style="width:16px;height:16px;object-fit:contain;vertical-align:middle;margin-right:6px;">
                                                    <?= htmlspecialchars((string) ($m['dosage'] ?? '-')) ?></p>
                                                <p class="instructions">
                                                    <?= htmlspecialchars((string) ($m['instructions'] ?? 'No extra instructions')) ?>
                                                </p>
                                            </div>
                                            <div class="med-status-block">
                                                <span
                                                    class="status-badge <?= htmlspecialchars($statusClass) ?>"><?= htmlspecialchars($status) ?></span>
                                            </div>
                                        </article>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                    ...
                    <?php else: ?>
                        <div class="card-panel empty-detail-panel">
                            <div class="empty-state-modern">
                                ...
                                <p>Choose a patient from the left panel to review their medication plan, key profile
                                    details, and today's progress.</p>
                                <button type="button" class="btn btn-primary" onclick="openAddModal()">Add Patient</button>
                            </div>
                        </div>
                    <?php endif; ?>
    ...
    <div id="addPatientModal" class="modal-overlay <?= $openAddModal ? 'is-open' : '' ?>"
        aria-hidden="<?= $openAddModal ? 'false' : 'true' ?>">
        <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="addPatientTitle">
            <button type="button" class="close-btn" onclick="closeAddModal()"
                aria-label="Close add patient modal">&times;</button>
            <div class="modal-copy">
                <div class="hero-kicker">Patient Link Request</div>
                <h3 id="addPatientTitle">Send a patient link request</h3>
                <p>Enter the patient's NIC exactly as it appears on their profile. Medora will prevent duplicate or
                    conflicting guardian links.</p>
            </div>
            <form action="<?= htmlspecialchars($base) ?>/guardian/patients/add" method="post"
                class="guardian-patient-form">
                <div class="field-group">
                    <label for="guardian-patient-nic">Patient NIC</label>
                    <input id="guardian-patient-nic" type="text" name="nic" required placeholder="e.g. 199512345678"
                        autocomplete="off">
                </div>
