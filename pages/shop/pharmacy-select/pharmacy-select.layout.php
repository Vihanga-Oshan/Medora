...
<head>
    ...
    <title>Select Pharmacy | Medora Shop</title>
...
<body>
    <div class="wrap">
        <section class="panel">
            <div class="toolbar">
                <h3 style="margin:0;">Choose a Pharmacy to Start Shopping</h3>
                <div style="display:flex; align-items:center; gap:10px;">
                    <?php if ((int) $selectedPharmacyId > 0): ?>
                        <a class="close-link" href="<?= htmlspecialchars($base) ?>/shop">Close</a>
                    <?php endif; ?>
            ...
            <div class="list">
                <?php foreach ($pharmacies as $p): ?>
                    <form method="post" class="item" data-lat="<?= htmlspecialchars((string) $p['latitude']) ?>"
                        data-lng="<?= htmlspecialchars((string) $p['longitude']) ?>" data-id="<?= (int) $p['id'] ?>">
                        <input type="hidden" name="action" value="select">
                        <h4><?= htmlspecialchars((string) $p['name']) ?></h4>
                        <p><?= htmlspecialchars((string) ($p['address_line1'] ?? '')) ?>,
                            <?= htmlspecialchars((string) ($p['city'] ?? '')) ?></p>
                        ...
                        <button class="btn <?= ((int) $selectedPharmacyId === (int) $p['id']) ? 'secondary' : '' ?>"
                            type="submit">
    ...
    <script>
        ...
        for (const p of pharmacies) {
            ...
            const marker = L.marker([p.lat, p.lng]).addTo(map).bindPopup(`<b>${p.name}</b><br>${p.address}`);
            byId[p.id] = marker;
        }
