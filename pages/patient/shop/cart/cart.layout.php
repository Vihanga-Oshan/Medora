...
<main class="container">
    <div class="order-header">
        <div>
            <a href="<?= htmlspecialchars($base) ?>/patient/shop" class="text-muted" style="text-decoration:none;">&larr; Back to Shop</a>
            <h1>Your Shopping Cart</h1>
        </div>
    </div>

    <?php if (!empty($flash['message'])): ?>
        <div class="card" style="padding:12px 16px; margin-bottom:16px; border-left:4px solid <?= ($flash['type'] ?? '') === 'error' ? '#dc3545' : '#28a745' ?>;">
            ...
            <div class="cart-items-list">
                <div class="card">
                    <?php foreach ($items as $item): ?>
                        <?php $m = $item['medicine']; ?>
                        <div class="cart-item">
                            <img src="<?= $toImageUrl((string)($m['image_path'] ?? '')) ?>" alt="<?= htmlspecialchars((string)($m['name'] ?? 'Medicine')) ?>" class="cart-item-img">
                            ...
                            <a href="<?= htmlspecialchars($base) ?>/patient/shop/cart/remove?id=<?= (int)($m['id'] ?? 0) ?>" class="cart-item-remove">Remove</a>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
