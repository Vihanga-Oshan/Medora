...
$iconAsset = static function (string $file, string $size = '20px') use ($base): string {
    $src = htmlspecialchars($base . '/assets/icons/' . $file, ENT_QUOTES, 'UTF-8');
    $size = htmlspecialchars($size, ENT_QUOTES, 'UTF-8');

    return '<img src="' . $src . '" alt="" aria-hidden="true" style="width:' . $size . ';height:' . $size . ';object-fit:contain;display:inline-block;vertical-align:middle;">';
};
$pillIcon = $iconAsset('pill.png', '24px');
$pillIconSm = $iconAsset('pill.png', '16px');
$checkIcon = $iconAsset('check-mark.png', '24px');
$checkIconLg = $iconAsset('check-mark.png', '28px');
$checkIconSm = $iconAsset('check-mark.png', '16px');
$clockIcon = $iconAsset('clock.png', '24px');
$dangerIcon = $iconAsset('danger.png', '24px');
$dangerIconSm = $iconAsset('danger.png', '16px');
$starIcon = $iconAsset('star.svg', '20px');
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="dashboard-wrapper">
            ...
            <div class="hero-content">
                ...
                <?php if (!empty($selectedPharmacy)): ?>
                    <div class="hero-info-glass">
                        ...
                        <a href="<?= htmlspecialchars($base) ?>/patient/pharmacy-select"
                            class="hero-inline-button">Change</a>
                    </div>
                <?php endif; ?>
            </div>
            <div class="hero-actions">
                <a href="<?= htmlspecialchars($base) ?>/patient/shop" class="btn btn-primary"><?= $pillIcon ?>Order
                    Medicines</a>
                <a href="<?= htmlspecialchars($base) ?>/patient/shop/orders" class="btn btn-outline"><?= $starIcon ?>My
                    Orders</a>
            </div>
        </header>
        ...
        <div class="main-layout">
            ...
            <section class="stats-overview">
                <div class="stat-card">
                    <div class="stat-icon-wrapper bg-blue"><?= $pillIcon ?></div>
                ...
                <div class="stat-card">
                    <div class="stat-icon-wrapper bg-green"><?= $checkIcon ?></div>
                    <div class="stat-details">
                ...
                <div class="stat-card">
                    <div class="stat-icon-wrapper bg-yellow"><?= $clockIcon ?></div>
                    <div class="stat-details">
                ...
                <div class="stat-card">
                    <div class="stat-icon-wrapper bg-red"><?= $dangerIcon ?></div>
                    <div class="stat-details">
            ...
            <div class="content-grid">
                ...
                <div class="schedule-column">
                    ...
                    <div class="section-header">
                        ...
                        <div class="date-badge"><?= $clockIcon ?><span>Today</span></div>
                    </div>
                            ...
                            <?php foreach ($pendingMedications as $m): ?>
                                <div class="medication-item">
                                    ...
                                    <div class="med-info-block">
                                        ...
                                        <p class="dosage-info"><?= $pillIconSm ?>         <?= htmlspecialchars($m['dosage']) ?></p>
                                        <p class="instructions"><?= htmlspecialchars($m['instructions'] ?? '') ?></p>
                                    </div>
                                    <div class="med-actions-block">
                                        <form action="<?= htmlspecialchars($base) ?>/patient/medications/mark" method="post">
                                            ...
                                            <input type="hidden" name="reminder_event_id"
                                                value="<?= (int) ($m['reminder_event_id'] ?? 0) ?>">
                                            <input type="hidden" name="patient_nic"
                                                value="<?= htmlspecialchars($user['nic']) ?>">
                                            <input type="hidden" name="status" value="TAKEN">
                                            <input type="hidden" name="time_slot"
                                                value="<?= htmlspecialchars((string) ($m['frequency_slot'] ?? $m['frequency'])) ?>">
                                            <input type="hidden" name="redirect"
                                                value="<?= htmlspecialchars($base) ?>/patient/dashboard">
                                            <button type="submit" class="action-btn btn-check"
                                                title="Mark as Taken"><?= $checkIconSm ?></button>
                                        </form>
                                        <form action="<?= htmlspecialchars($base) ?>/patient/medications/mark" method="post">
                                            <input type="hidden" name="schedule_id" value="<?= (int) $m['id'] ?>">
                                            <input type="hidden" name="reminder_event_id"
                                            ...
                                            <button type="submit" class="action-btn btn-cross"
                                                title="Mark as Missed"><?= $dangerIconSm ?></button>
                                        </form>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                ...
                <div class="timetable-column">
                    ...
                    <div class="card-panel">
                        ...
                        <form method="get" class="modern-date-selector">
                            ...
                            <div class="input-group">
                                ...
                                <button type="submit" class="btn-icon" aria-label="Search timetable">Search</button>
                            </div>
                        </form>
                        <div class="timetable-results">
                            <p class="results-header">Schedule for
                                    ...
                                    <?php foreach ($medications as $m):
                                        ...
                                        <div class="timetable-row">
                                            <div class="row-info">
                                                ...
                                                <span><?= htmlspecialchars($m['dosage']) ?> &bull;
                                                    <?= htmlspecialchars($m['frequency']) ?></span>
                                            </div>
                                            <span
                                                class="status-pill <?= $pillClass ?>"><?= htmlspecialchars(strtoupper($m['status'])) ?></span>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php endif; ?>
                    ...
                    <?php if ($requestingGuardian): ?>
                        <div class="card-panel alert-panel">
                            <h4 class="alert-title">&#128276; Guardian Request</h4>
                            <p><strong><?= htmlspecialchars($requestingGuardian['name']) ?></strong> wants to be linked as
                                your guardian.</p>
                            <div class="alert-actions">
                                <form action="<?= htmlspecialchars($base) ?>/patient/guardian/accept" method="post"
                                    style="display:inline;">
                                    <input type="hidden" name="guardian_nic"
                                        value="<?= htmlspecialchars($requestingGuardian['nic']) ?>">
                                    <button type="submit" class="btn btn-accept">Accept</button>
                                </form>
                                <form action="<?= htmlspecialchars($base) ?>/patient/guardian/reject" method="post"
                                    style="display:inline;">
                                    <input type="hidden" name="guardian_nic"
                                        value="<?= htmlspecialchars($requestingGuardian['nic']) ?>">
                                ...
                                <?php foreach ($notifications as $n): ?>
                                    <li class="notification-item">
                                        <div class="notif-header">
                                            <span class="notif-label">&#128276; Alert</span>
