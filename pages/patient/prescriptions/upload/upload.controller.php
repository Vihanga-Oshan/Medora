...
if (Request::isPost()) {
    $formData = [
        'wants_medicine_order' => isset($_POST['wants_medicine_order']) && $_POST['wants_medicine_order'] === '1' ? '1' : '0',
        'wants_schedule' => isset($_POST['wants_schedule']) && $_POST['wants_schedule'] === '1' ? '1' : '0',
    ...
    $billingName = trim((string) ($user['name'] ?? ''));
    $billingEmail = trim((string) ($user['email'] ?? ''));
    if ($billingEmail === '' && !empty($user['nic'])) {
        $emailBase = preg_replace('/[^a-zA-Z0-9]/', '', (string) $user['nic']) ?: 'patient';
        $billingEmail = strtolower($emailBase) . '@medora.local';
    }
