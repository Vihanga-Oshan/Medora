...
<body class="admin-body">
    <aside class="sidebar">
        ...
        <ul class="nav-links">
            ...
            <li class="active"><a href="<?= htmlspecialchars($base) ?>/admin/pharmacist-requests"><i>&#128221;</i>
                    Requests</a></li>
        ...
        <section class="section-container">
            ...
            <section class="panel-card requests-card">
                <div class="filter-pills">
                    <a class="btn btn-small<?= $statusFilter === 'pending' ? ' btn-primary' : ' btn-muted' ?>"
                        href="<?= htmlspecialchars($base) ?>/admin/pharmacist-requests?status=pending">Pending</a>
                    <a class="btn btn-small<?= $statusFilter === 'approved' ? ' btn-primary' : ' btn-muted' ?>"
                        href="<?= htmlspecialchars($base) ?>/admin/pharmacist-requests?status=approved">Approved</a>
                    <a class="btn btn-small<?= $statusFilter === 'rejected' ? ' btn-primary' : ' btn-muted' ?>"
                        href="<?= htmlspecialchars($base) ?>/admin/pharmacist-requests?status=rejected">Rejected</a>
                </div>
                    ...
                    <tbody>
                        ...
                        <?php foreach ($requests as $r): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars((string) $r['email']) ?><br><small><?= htmlspecialchars((string) ($r['phone'] ?? '')) ?></small>
                                </td>
                                <td><?= htmlspecialchars((string) $r['license_no']) ?></td>
                                <td><?= htmlspecialchars((string) ($r['pharmacy_name'] ?? 'Any')) ?></td>
                                <td><?= htmlspecialchars((string) $r['status']) ?><br><small><?= htmlspecialchars((string) ($r['note'] ?? '')) ?></small>
                                </td>
                                <td>
