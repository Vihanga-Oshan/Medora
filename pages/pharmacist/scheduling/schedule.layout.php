    ...
    <div class="container">
        ...
        <main class="main-content">
            ...
            <div style="padding: 28px 34px;">
                ...
                <?php if ($errorMessage !== ''): ?>
                    <div
                        style="margin:0 0 14px 0; padding:10px 12px; border:1px solid #fecaca; background:#fef2f2; color:#991b1b; border-radius:8px;">
                        <?= htmlspecialchars($errorMessage) ?>
                    </div>
                <?php endif; ?>

                <div class="scheduling-grid">
                    <form action="<?= htmlspecialchars($base) ?>/pharmacist/scheduling" method="post"
                        id="schedulingForm" class="schedule-form">
                        <input type="hidden" name="prescription_id" value="<?= (int) ($p['id'] ?? 0) ?>">
                        <input type="hidden" name="patient_nic"
                            value="<?= htmlspecialchars((string) ($p['patient_nic'] ?? $p['patientNic'] ?? '')) ?>">
                                ...
                                <div class="med-row" data-row>
                                    ...
                                    <select name="medicineId[]" required>
                                        ...
                                        <?php foreach ($medicines as $m): ?>
                                            <option value="<?= (int) ($m['id'] ?? 0) ?>">
                                                <?= htmlspecialchars((string) ($m['name'] ?? '')) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    ...
                                    <select name="dosageId[]" required>
                                        <?php foreach ($dosages as $d): ?>
                                            <option value="<?= (int) ($d['id'] ?? 0) ?>">
                                                <?= htmlspecialchars((string) ($d['label'] ?? '')) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    ...
                                    <select name="frequencyId[]" required>
                                        <?php foreach ($frequencies as $f): ?>
                                            <option value="<?= (int) ($f['id'] ?? 0) ?>">
                                                <?= htmlspecialchars((string) ($f['label'] ?? '')) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    ...
                                    <select name="mealTimingId[]">
                                        ...
                                        <?php foreach ($mealTimings as $mt): ?>
                                            <option value="<?= (int) ($mt['id'] ?? 0) ?>">
                                                <?= htmlspecialchars((string) ($mt['label'] ?? '')) ?></option>
                                        <?php endforeach; ?>
                                    </select>
                        ...
                        <?php if ($wantsMedicineOrder): ?>
                            <div class="workflow-section">
                                ...
                                <p class="workflow-copy">Select the inventory items the pharmacy will prepare for this
                                    patient.</p>
                                <div id="orderRows">
                                    <div class="med-row order-row" data-order-row>
                                        ...
                                        <select name="orderMedicineId[]" required>
                                            ...
                                            <?php foreach ($inventoryMedicines as $medicine): ?>
                                                <option value="<?= (int) ($medicine['id'] ?? 0) ?>">
                                                    <?= htmlspecialchars((string) ($medicine['name'] ?? 'Medicine')) ?> | Stock
                                                    <?= (int) ($medicine['quantity_in_stock'] ?? 0) ?> | Rs.
                                                    <?= number_format((float) ($medicine['price'] ?? 0), 2) ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        ...
                                        <button type="button" class="btn-reject"
                                            onclick="removeOrderRow(this)">Remove</button>
                                        <hr>
                                    </div>
                                </div>
                                    ...
                                    <div class="existing-order-list">
                                        ...
                                        <?php foreach ($orderItems as $item): ?>
                                            <div class="existing-order-item">
                                                <?= htmlspecialchars((string) ($item['medicine_name'] ?? 'Medicine')) ?> x
                                                <?= (int) ($item['quantity'] ?? 0) ?>
                                            </div>
                                        <?php endforeach; ?>
                                    </div>
                    ...
                    <div class="patient-details-box">
                        ...
                        <div class="workflow-summary">
                            <?php if ($wantsMedicineOrder): ?><span class="workflow-pill">Medicine
                                    Order</span><?php endif; ?>
                            <?php if ($wantsSchedule): ?><span
                                    class="workflow-pill workflow-pill-secondary">Schedule</span><?php endif; ?>
                        </div>
                        <div class="preview-placeholder">
                            <?php if ($isPdf): ?>
                                <div class="pdf-box">
                                    <span class="pdf-icon">&#128196;</span>
                                    <a href="<?= htmlspecialchars($base) ?>/prescriptions/file?id=<?= (int) ($p['id'] ?? 0) ?>"
                                        target="_blank" class="view-link">View PDF</a>
                                </div>
                            <?php else: ?>
                                <img src="<?= htmlspecialchars($base) ?>/prescriptions/file?id=<?= (int) ($p['id'] ?? 0) ?>"
