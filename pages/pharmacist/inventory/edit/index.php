...
<head>
    ...
    <script src="<?= htmlspecialchars($base) ?>/assets/js/inventory-validation.js"></script>
</head>
        ...
        <main class="main-content">
            <header class="header">
                ...
                <div class="greeting"><span class="greeting-icon">&#9998;</span>
                    <div><span class="greeting-text">Edit Medicine</span><span class="date-time">Modify medicine
                            details</span></div>
                </div>
            </header>
            ...
            <section class="form-section">
                <div class="section-header">
                    <div>
                        <h2>Edit Medicine</h2>
                        <p>Modify details and save changes.</p>
                    </div>
                    <a class="add-btn"
                        href="<?= htmlspecialchars($base) ?>/pharmacist/medicine-inventory"><span>&larr;</span> Back to
                        Inventory</a>
                </div>
                ...
                <form action="" method="post" enctype="multipart/form-data" class="styled-form" id="medicineForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="brand_new"
                            value="<?= htmlspecialchars((string) ($_POST['brand_new'] ?? '')) ?>"
                            placeholder="Type new brand name">
                    </div>
                    <div class="form-group"><label>Generic Name</label><input type="text" name="generic_name"
                            value="<?= htmlspecialchars($fv('generic_name')) ?>"></div>
                    <div class="form-group"><label>MEDICINE NAME <span style="color:#dc2626;">*</span></label><input
                            type="text" name="med_name" value="<?= htmlspecialchars($fv('med_name')) ?>"
                            placeholder="e.g. Lipitor" required></div>
                    <div class="form-group">
                        ...
                        <input type="text" name="manufacturer_new"
                            value="<?= htmlspecialchars((string) ($_POST['manufacturer_new'] ?? '')) ?>"
                            placeholder="Type new manufacturer">
                    </div>
                    <div class="form-group">
                        ...
                        <select name="supplier_existing">
                            ...
                            <?php foreach ($suppliers as $supplier): ?>
                                <option value="<?= (int) ($supplier['id'] ?? 0) ?>"
                                    <?= $selectedSupplier === (string) ($supplier['id'] ?? '') ? 'selected' : '' ?>>
                        ...
                        <input type="text" name="supplier_new"
                            value="<?= htmlspecialchars((string) ($_POST['supplier_new'] ?? '')) ?>"
                        ...
                        <?php if (!empty($categories)): ?>
                            ...
                            <select name="category_id">
                                ...
                                <?php foreach ($categories as $cat): ?>
                                    ...
                                    <option value="<?= $cid ?>" <?= ($selectedCategoryId === (string) $cid) ? 'selected' : '' ?>>
                                        <?= htmlspecialchars((string) ($cat['name'] ?? '')) ?></option>
                                <?php endforeach; ?>
                            </select>
                        <?php else: ?>
                            <input type="text" name="category" value="<?= htmlspecialchars($fv('category')) ?>"
                                placeholder="Category name">
                        <?php endif; ?>
                    </div>
                    <div class="form-group full-width"><label>Description</label><textarea
                            name="description"><?= htmlspecialchars($fv('description')) ?></textarea></div>
                    ...
                    <div class="form-group"><label>Contact Person</label><input type="text"
                            ...
                            value="<?= htmlspecialchars((string) ($_POST['supplier_contact_person'] ?? ($medicine['supplier_contact_person'] ?? ''))) ?>">
                    </div>
                    <div class="form-group"><label>Supplier Phone</label><input type="text" name="supplier_phone"
                            value="<?= htmlspecialchars((string) ($_POST['supplier_phone'] ?? ($medicine['supplier_phone'] ?? ''))) ?>">
                    </div>
                    <div class="form-group"><label>Supplier Email</label><input type="email" name="supplier_email"
                            value="<?= htmlspecialchars((string) ($_POST['supplier_email'] ?? ($medicine['supplier_email'] ?? ''))) ?>">
                    </div>
                    <div class="form-group"><label>Lead Time (Days)</label><input type="number"
                            name="supplier_lead_time_days" min="0"
                            value="<?= htmlspecialchars((string) ($_POST['supplier_lead_time_days'] ?? ($medicine['supplier_lead_time_days'] ?? '0'))) ?>">
                    </div>
                    <div class="form-group full-width"><label>Supplier Address</label><textarea
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="dosage_form_new"
                            value="<?= htmlspecialchars((string) ($_POST['dosage_form_new'] ?? '')) ?>"
                            placeholder="Type new dosage form">
                    </div>
                    <div class="form-group"><label>Strength <span style="color:#dc2626;">*</span></label><input
                            type="text" name="strength" required value="<?= htmlspecialchars($fv('strength')) ?>"></div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="selling_unit_new"
                            value="<?= htmlspecialchars((string) ($_POST['selling_unit_new'] ?? '')) ?>"
                            placeholder="Type new selling unit">
                    </div>
                    <div class="form-group"><label>Doses per Unit</label><input type="number" name="unit_quantity"
                            min="1" value="<?= htmlspecialchars($fv('unit_quantity', '1')) ?>"></div>
                    <div class="form-group"><label>Current Stock (Units)</label><input type="number"
                            name="quantity_in_stock" min="0"
                            value="<?= htmlspecialchars($fv('quantity_in_stock', '0')) ?>"></div>
                    <div class="form-group"><label>Low Stock Threshold</label><input type="number"
                            name="low_stock_threshold" min="0"
                            value="<?= htmlspecialchars($fv('low_stock_threshold', '10')) ?>" required></div>
                    <div class="form-group"><label>Reorder Quantity</label><input type="number" name="reorder_quantity"
                            min="0" value="<?= htmlspecialchars($fv('reorder_quantity', '25')) ?>"></div>
                    <div class="form-group"><label>Price per Unit <span style="color:#dc2626;">*</span></label><input
                            type="number" name="price" step="0.01" min="0"
                            value="<?= htmlspecialchars($fv('price', '0')) ?>" required></div>
                    <div class="form-group"><label>Unit Cost</label><input type="number" name="unit_cost" step="0.01"
                            min="0" value="<?= htmlspecialchars($fv('unit_cost', $fv('price', '0'))) ?>"></div>
                    ...
                    <div class="form-group"><label>Expiry Date</label><input type="date" name="expiry_date"
                            value="<?= htmlspecialchars(substr($fv('expiry_date'), 0, 10)) ?>"></div>
                    <div class="form-group"><label>Batch Number</label><input type="text" name="batch_number"
                            value="<?= htmlspecialchars($fv('batch_number')) ?>" placeholder="Batch / lot number"></div>
                    <div class="form-group">
                        <label>Medicine Image</label>
                        <input type="file" name="imageFile" accept="image/*">
                        <?php if (!empty($medicine['image_path'])): ?>
                            <small style="color:#64748b;">Current:
                                <?= htmlspecialchars((string) $medicine['image_path']) ?></small>
                        <?php endif; ?>
                    </div>

                    <div class="btn-group"><button type="submit" class="btn-submit">Update Medicine Details</button>
                    </div>
                </form>
            </section>
        </main>
