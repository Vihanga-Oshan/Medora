...
<div class="auth-shell">
    <section class="auth-form-panel">
        <div class="login-container">
            ...
            <?php if ($success): ?><p style="color:#2e7d32;margin-bottom:12px;"><?= htmlspecialchars($success) ?></p><?php endif; ?>

            <form method="post">
                <label>Full Name</label>
                <input type="text" name="name" required value="<?= htmlspecialchars(Request::post('name') ?? '') ?>">

                <label>Email</label>
                <input type="email" name="email" required value="<?= htmlspecialchars(Request::post('email') ?? '') ?>">
                ...
                <select name="requested_pharmacy_id" required style="height:42px;width:100%;border:1px solid #cfd7e5;border-radius:9px;padding:0 10px;margin-bottom:11px;">
                    ...
                    <?php foreach ($pharmacies as $ph): ?>
                        <option value="<?= (int)$ph['id'] ?>" <?= ((int)(Request::post('requested_pharmacy_id') ?? 0) === (int)$ph['id']) ? 'selected' : '' ?>>
                            <?= htmlspecialchars((string)$ph['name']) ?> - <?= htmlspecialchars((string)($ph['city'] ?? '')) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
    ...
    <aside class="auth-visual-panel">
        <div class="visual-card">
            <img src="<?= htmlspecialchars($base) ?>/assets/img/counselor-login.png" alt="Pharmacist onboarding">
